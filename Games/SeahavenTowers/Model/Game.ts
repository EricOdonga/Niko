...
export class Game extends GameBase implements IGame {
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        // Deal 5 cards face up to each of the 10 tableau columns
        for (let i = 0; i < 50; ++i) {
            const card = tempStock.peek();
            if (card) {
                card.faceUp = true;
                const pile = this.tableaux[i % TABLEAUX_COUNT] ?? Debug.error();
                pile.push(card);
                yield DelayHint.Quick;
            }
        }

        // Place remaining 2 cards face up directly into stacks 10 and 11
        for (let i = 0; i < 2; ++i) {
            const card = tempStock.peek();
            if (card) {
                card.faceUp = true;
                const pile = this.freeCells[i] ?? Debug.error();
                pile.push(card);
                yield DelayHint.Quick;
            }
        }
    ...
    private isTableauxDrop_(card: Card, pile: Pile) {
        if (!this.isTableauxDropSource_(card) && !this.isFreeCellDropSource_(card) && !this.isFoundationDropSource_(card)) {
            return false;
        }
    ...
    private isTableauxDropSource_(card: Card) {
        if (this.tableaux.indexOf(card.pile) >= 0 && card.faceUp) {
            for (let i = card.pile.length - 1; i > card.pileIndex; i--) {
                ...
                if (card0.faceUp && card1.faceUp && card0.suit === card1.suit && this.getCardValue_(card0) === this.getCardValue_(card1) + 1) {
                    // valid same suit sequence
                } else {
                    return false;
                }
            }
