...
export class Game extends GameBase implements IGame {
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        for (let i = 0; i < 52; ++i) {
            ...
            if (card) {
                card.faceUp = true;
                this.tableaux[i % TABLEAUX_COUNT].push(card);
                yield DelayHint.Quick;
            }
        }
    ...
    private *doAutoMoves_() {
        ...
        while (moved) {
            ...
            // Find lowest foundation value
            let minFoundationValue = 13;
            for (const pile of this.foundations) {
                const topCard = pile.peek();
                const val = topCard ? this.getCardValue_(topCard) : 0;
                if (val < minFoundationValue) {
                    minFoundationValue = val;
                }
            }
            ...
            for (const card of topCards) {
                for (const pile of this.foundations) {
                    if (this.isFoundationDrop_(card, pile)) {
                        ...
                        // Safely auto-play cards up to minFoundationValue + 1 (or +2 if different color logic is considered, but usually +1 for Freecell)
                        // Actually in Freecell, it's safe to auto play if card value <= minFoundationValue + 1 (for black) if it's red, 
                        // Wait, a safe auto move is if card value <= minFoundation value + 1?
                        // Let's just do minFoundationValue + 2, actually.
                        if (val <= minFoundationValue + 2) {
                            yield* this.doFoundationDrop_(card, pile);
                            moved = true;
