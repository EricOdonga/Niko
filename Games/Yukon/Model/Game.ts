...
export class Game extends GameBase implements IGame {
    ...
    private isTableauxDrop_(card: Card, pile: Pile) {
        ...
        if (this.tableaux.indexOf(pile) >= 0) {
            ...
            if (topCard) {
                if (topCard.rank === card.rank + 1 && topCard.colour !== card.colour) {
                    return true;
                }
            } else {
    ...
    private isTableauxDropSource_(card: Card) {
        if (this.dragSingleSources_.indexOf(card.pile) >= 0 && card.pile.peek() === card && card.faceUp) {
            ...
            // Yukon rule: any face up card can be dragged along with all cards on top of it
            return true;
        }
        return false;
    }
    ...
    private isSafeToAutoMoveToFoundation_(card: Card) {
        ...
        if (cardValue <= 2) return true; // Aces and Twos are always safe

        // Find the lowest value of the opposite color in foundations
        let lowestOpposite = 14;
        for (const foundation of this.foundations) {
            const topCard = foundation.peek();
            if (topCard) {
                if (topCard.colour !== card.colour) {
                    lowestOpposite = Math.min(lowestOpposite, topCard.rank);
                }
            } else {
                lowestOpposite = 0; // Empty foundation
            }
        }

        // It is safe if the card is not needed to build on tableaus.
        // A red 3 is safe if black 2s are already in the foundation.
        return cardValue <= lowestOpposite + 1;
    }
}
